...
from .error import ConfigFileError, BestAirException
from .pkg_utils import getResource

DEFAULT_SECTION = 'DEFAULT'
USR_CONFIG_FILE = 'best_air.cfg'
...
#
[DEFAULT]
BEST_AIR.DefaultProject = my_project
BEST_AIR.LogLevel = INFO
BEST_AIR.LogConsole = True
BEST_AIR.ShowStackTrace = True

[my_project]
# BEST_AIR.ClassPath = %(Home)s/my_directory/my_procs.py
BEST_AIR.StreamComponents = foo, bar , baz
"""
...
# may not be needed in BEST-AIR
# def parse_version_info(vers=None):
#     import semver
#
#     vers = vers or getParam('BEST_AIR.VersionNumber')
#
...
def ensure_default_config():
    ...
    if not os.path.lexists(configPath) or os.stat(configPath).st_size == 0:
        try:
            ...
            raise BestAirException(f'\n***\n*** Failed to write default BEST-AIR configuration file {configPath}: {e}.\n***\n')
...
def _readConfigResourceFile(filename, package='best_air', raiseError=True):
    try:
...
def getHomeDir():
    ...
    if PlatformName == 'Windows':
        ...
        # If set, we use BEST_AIR_HOME to identify the folder with the config file;
        # otherwise, we use HOMESHARE if set, or HOMEPATH, in that order.
        homedir = env.get('BEST_AIR_HOME') or env.get('HOMESHARE') or env.get('HOMEPATH')
        ...
        home = env.get('BEST_AIR_HOME') or os.getenv('HOME')
...
def readConfigFiles(allowMissing=False):
    """
    Read the BEST_AIR configuration files, starting with ``best_air/etc/system.cfg``,
    followed by ``best_air/etc/{platform}.cfg`` if present. If the environment variable
    ``PYGCAM_SITE_CONFIG`` is defined, its value should be a config file, which is
    read next. Finally, the user's config file, ``~/best_air.cfg``, is read. Each
    ...
    siteConfig = os.getenv('BEST_AIR_SITE_CONFIG')
    if siteConfig:
        try:
    ...
    # Customizations are stored in ~/best_air.cfg
    ...
    # Dynamically set (if not defined) BEST_AIR.ProjectName in each section, holding the
    # section (i.e., project) name. If user has set this, the value is unchanged.
    projectNameVar = 'BEST_AIR.ProjectName'
    ...
    projectName = getParam('BEST_AIR.DefaultProject', section=DEFAULT_SECTION)
    if projectName:
...
def getParam(name, section=None, raw=False, raiseError=True):
    ...
    if not section:
        raise BestAirException('getParam was called without setting "section"')
    ...
    try:
        ...
        if raiseError:
            raise BestAirException('getParam: unknown section "%s"' % section)
        else:
            return None

    except configparser.NoOptionError:
        if raiseError:
            raise BestAirException('getParam: unknown variable "%s"' % name)
        else:
            return None

    # Deprecated (docker)
    # perform pathname translation for use of best_air.cfg in Docker images
    # if _PathMap:
...
def getParamAsBoolean(name, section=None):
    ...
    :raises: :py:exc:`best_air.error.ConfigFileError`
    """
