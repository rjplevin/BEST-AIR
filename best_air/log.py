...
.. Logging support.
   ...
   logger = best_air.log.getLogger(__name__) as a top-level statement, evaluated
...
def parseLevels(levelStr=None):
    ...
    is equivalent to "best_air.config".
    ...
    :param levelStr: a comma-delimited string of module:logLevel values. If
        no ':' is present, the value is treated as the default logLevel for BEST_AIR.
        If levelStr is None, the value of the variable 'BEST_AIR.LogLevel' is used.
    ...
    levelStr = levelStr or getParam('BEST_AIR.LogLevel')
...
def _configureLogger(name, force=False):
    ...
    global _LogLevels
    if not _LogLevels:
        setLogLevels(getParam('BEST_AIR.LogLevel') or 'WARN')
    ...
    logConsole = getParamAsBoolean('BEST_AIR.LogConsole')
    if logConsole:
        consoleFormat = getParam('BEST_AIR.LogConsoleFormat')
        _addHandler(logger, consoleFormat)

    logFile = getParam('BEST_AIR.LogFile')
    if logFile:
        fileFormat = getParam('BEST_AIR.LogFileFormat')
